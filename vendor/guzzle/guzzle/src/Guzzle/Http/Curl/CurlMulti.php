...
class CurlMulti extends AbstractHasDispatcher implements CurlMultiInterface {
	...
	public function add(RequestInterface $request) {
		...
		$this->dispatch(self::ADD_REQUEST, ['request' => $request]);
	...
	public function remove(RequestInterface $request) {
		...
		if (($index = array_search($request, $this->requests, true)) !== false) {
			...
			$this->dispatch(self::REMOVE_REQUEST, ['request' => $request]);
			return true;
		}
	...
	public function reset($hard = false) {
		...
		$this->requests = $this->resourceHash = $this->exceptions = $this->successful = [];
	}
	...
	protected function perform() {
		$event = new Event(['curl_multi' => $this]);
	...
	protected function processResponse(RequestInterface $request, CurlHandle $handle, array $curl) {
		...
		$state = $request->setState(RequestInterface::STATE_ERROR, ['exception' => $curlException]);
		...
		if (RequestInterface::STATE_ERROR == $state) {
	...
	protected function removeErroredRequest(RequestInterface $request, \Exception $e = null) {
		$this->exceptions[] = ['request' => $request, 'exception' => $e];
		$this->remove($request);
		$this->dispatch(self::MULTI_EXCEPTION, ['exception' => $e, 'all_exceptions' => $this->exceptions]);
	}
