...
class RequestMediator {
	...
	public function progress($downloadSize, $downloaded, $uploadSize, $uploaded, $handle = null) {
		$this->request->dispatch('curl.callback.progress', [
	...
	public function readRequestBody($ch, $fd, $length) {
		...
		if ($this->emitIo) {
			$this->request->dispatch('curl.callback.read', ['request' => $this->request, 'read' => $read]);
		}
	...
	public function receiveResponseHeader($curl, $header) {
		...
		if (strpos($header, 'HTTP/') === 0) {
			...
			$this->request->dispatch('request.receive.status_line', [
				'request' => $this,
				'line' => $header,
				'status_code' => $code,
	...
	public function writeResponseBody($curl, $write) {
		if ($this->emitIo) {
			$this->request->dispatch('curl.callback.write', [
				'request' => $this->request,
				'write' => $write,
			]);
		}
