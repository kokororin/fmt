...
class EntityEnclosingRequest extends Request implements EntityEnclosingRequestInterface {
	...
	public function setBody($body, $contentType = null) {
		...
		if (null === $contentType && !$this->hasHeader('Content-Type')) {
		...
		if (!$this->body->isSeekable() && false !== $this->expectCutoff) {
			$this->setHeader('Expect', '100-Continue');
		}

		// Set the Content-Length header if it can be determined
		$size = $this->body->getContentLength();
		if (null !== $size && false !== $size) {
	...
	public function setState($state, array $context = []) {
		...
		if (self::STATE_TRANSFER == $state && !$this->body && !count($this->postFields) && !count($this->postFiles)) {
