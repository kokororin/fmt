...
class Request extends AbstractMessage implements RequestInterface {
	...
	public function __clone() {
		...
		$this->dispatch('request.clone', ['request' => $this]);
	}
	...
	public static function onRequestError(Event $event) {
		...
		$event['request']->setState(self::STATE_ERROR, ['exception' => $e] + $event->toArray());
		throw $e;
	}
	...
	public function setEventDispatcher(EventDispatcherInterface $eventDispatcher) {
		...
		$this->eventDispatcher->addListener('request.error', [__CLASS__, 'onRequestError'], -255);
	...
	public function setState($state, array $context = []) {
		...
		case self::STATE_TRANSFER:
			if ($oldState !== $state) {
				...
				$this->dispatch('request.before_send', ['request' => $this]);
			}
			break;
		...
		case self::STATE_ERROR:
			if (isset($context['exception'])) {
				$this->dispatch('request.exception', [
					'request' => $this,
	 ...
	 */
	protected function processResponse(array $context = []) {
		if (!$this->response) {
		...
		if (RequestInterface::STATE_COMPLETE == $this->state) {
