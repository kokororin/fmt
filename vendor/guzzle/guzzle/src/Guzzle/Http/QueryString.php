...
class QueryString extends Collection {
	...
	public function encodeValue($value) {
		if (self::RFC_3986 == $this->urlEncode) {
			return rawurlencode($value);
		} elseif (self::FORM_URLENCODED == $this->urlEncode) {
	...
	public function useUrlEncoding($encode) {
		$this->urlEncode = (true === $encode) ? self::RFC_3986 : $encode;
	...
	private function convertKvp($name, $value) {
		if (self::BLANK === $value || null === $value || false === $value) {
			return $name;
		} elseif (!is_array($value)) {
