...
class UriTemplate implements UriTemplateInterface {
	...
	public function expand($template, array $variables) {
		if (self::DEFAULT_PATTERN == $this->regex && false === strpos($template, '{')) {
			return $template;
		}
		...
		return preg_replace_callback($this->regex, [$this, 'expandMatch'], $this->template);
	}
	...
	private function expandMatch(array $matches) {
		...
		foreach ($parsed['values'] as $value) {
			if (!array_key_exists($value['value'], $this->variables) || null === $this->variables[$value['value']]) {
				continue;
			}
