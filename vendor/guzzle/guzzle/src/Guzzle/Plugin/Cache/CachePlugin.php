...
class CachePlugin implements EventSubscriberInterface {
	...
	public function __construct($options = null) {
		if (!is_array($options)) {
			if ($options instanceof CacheAdapterInterface) {
				$options = ['storage' => new DefaultCacheStorage($options)];
			} elseif ($options instanceof CacheStorageInterface) {
				$options = ['storage' => $options];
			} elseif ($options) {
				$options = ['storage' => new DefaultCacheStorage(CacheAdapterFactory::fromCache($options))];
	...
	public function canResponseSatisfyRequest(RequestInterface $request, Response $response) {
		...
		if ($response->isFresh() === false) {
			...
			if (null !== $maxStale) {
				if (true !== $maxStale && $response->getFreshness() < (-1 * $maxStale)) {
					return false;
				}
	...
	protected function addResponseHeaders(RequestInterface $request, Response $response) {
		...
		$lookup = (true === $params['cache.lookup'] ? 'HIT' : 'MISS') . ' from GuzzleCache';
		...
		if (true === $params['cache.hit']) {
			$xcache = 'HIT from GuzzleCache';
		} elseif ('error' == $params['cache.hit']) {
