...
abstract class AbstractCommand extends Collection implements CommandInterface {
	 ...
	 */
	public function __construct($parameters = [], OperationInterface $operation = null) {
	...
	public function isExecuted() {
		return null !== $this->request && $this->request->getState() == 'complete';
	}
	...
	public function prepare() {
		if (!$this->isPrepared()) {
			...
			$this->client->dispatch('command.before_prepare', ['command' => $this]);
			...
			$this->client->dispatch('command.after_prepare', ['command' => $this]);
		}
	...
	protected function createOperation() {
		return new Operation(['name' => get_class($this)]);
	}
	...
	protected function process() {
		$this->result = self::TYPE_RAW != $this[self::RESPONSE_PROCESSING] ? DefaultResponseParser::getInstance()->parse($this)
