...
class DefaultRequestSerializer implements RequestSerializerInterface {
	...
	protected function createRequest(CommandInterface $command) {
		...
		$options = $command[AbstractCommand::REQUEST_OPTIONS] ?: [];
		...
		return $client->createRequest($operation->getHttpMethod(), [$uri, $variables], null, null, $options);
	}
	...
	protected function prepareAdditionalParameters(
		...
		foreach ($command->toArray() as $key => $value) {
			...
			if (null !== $value && !in_array($key, $hidden)
