...
class SchemaValidator implements ValidatorInterface {
	...
	protected function determineType($type, $value) {
		foreach ((array) $type as $t) {
			if ('string' == $t && (is_string($value) || (is_object($value) && method_exists($value, '__toString')))) {
				return 'string';
			} elseif ('object' == $t && (is_array($value) || is_object($value))) {
				return 'object';
			} elseif ('array' == $t && is_array($value)) {
				return 'array';
			} elseif ('integer' == $t && is_integer($value)) {
				return 'integer';
			} elseif ('boolean' == $t && is_bool($value)) {
				return 'boolean';
			} elseif ('number' == $t && is_numeric($value)) {
				return 'number';
			} elseif ('numeric' == $t && is_numeric($value)) {
				return 'numeric';
			} elseif ('null' == $t && !$value) {
				return 'null';
			} elseif ('any' == $t) {
	...
	protected function recursiveProcess(Parameter $param, &$value, $path = '', $depth = 0) {
		...
		} elseif ('array' == $type && $valueIsArray && $param->getItems()) {
		...
		if ($required && null === $value && 'null' != $type) {
		...
		} elseif ('integer' == $type || 'number' == $type || 'numeric' == $type) {
