...
class ClosureCacheAdapterTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	protected function setUp() {
		...
		$that = $this;
		$this->callables = [
			'contains' => function ($id, $options = []) use ($that) {
				return array_key_exists($id, $that->data);
			},
			'delete' => function ($id, $options = []) use ($that) {
				unset($that->data[$id]);
				return true;
			},
			'fetch' => function ($id, $options = []) use ($that) {
				return array_key_exists($id, $that->data) ? $that->data[$id] : null;
			},
			'save' => function ($id, $data, $lifeTime, $options = []) use ($that) {
				$that->data[$id] = $data;
				return true;
			},
		];
