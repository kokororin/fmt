...
class BatchTransferExceptionTest extends \Guzzle\Tests\GuzzleTestCase {
	public function testContainsBatch() {
		...
		$transferException = new BatchTransferException(['foo'], [1, 2], $e, $t, $d);
		$this->assertEquals(['foo'], $transferException->getBatch());
		...
		$this->assertEquals([1, 2], $transferException->getTransferredItems());
	}
}
