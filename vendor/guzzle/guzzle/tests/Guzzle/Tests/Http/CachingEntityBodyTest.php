...
class CachingEntityBodyTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testWrapsContentEncoding() {
		$a = $this->getMockBuilder('Guzzle\Http\EntityBody')
			->setMethods(['getContentEncoding'])
			->setConstructorArgs([fopen(__FILE__, 'r')])
			->getMock();
		$a->expects($this->once())
	...
	public function testWrapsCustomData() {
		$a = $this->getMockBuilder('Guzzle\Http\EntityBody')
			->setMethods(['getCustomData', 'setCustomData'])
			->setConstructorArgs([fopen(__FILE__, 'r')])
			->getMock();
	...
	public function testWrapsMetadata() {
		$a = $this->getMockBuilder('Guzzle\Http\EntityBody')
			->setMethods(['getMetadata', 'getWrapper', 'getWrapperData', 'getStreamType', 'getUri'])
			->setConstructorArgs([fopen(__FILE__, 'r')])
			->getMock();
		...
		$d = new CachingEntityBody($a);
		$this->assertEquals([], $d->getMetaData());
		$this->assertEquals('wrapper', $d->getWrapper());
		$this->assertEquals([], $d->getWrapperData());
