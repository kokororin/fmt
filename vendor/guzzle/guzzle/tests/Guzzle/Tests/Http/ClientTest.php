...
class ClientTest extends \Guzzle\Tests\GuzzleTestCase {
	public function testAcceptsConfig() {
		...
		$this->assertSame($client, $client->setConfig([
			'test' => '123',
		]));
		$this->assertEquals(['test' => '123'], $client->getConfig()->getAll());
	...
	public function testAllowsCustomCurlMultiObjects() {
		$mock = $this->getMock('Guzzle\\Http\\Curl\\CurlMulti', ['add', 'send']);
	...
	public function testAllowsCustomVariablesWhenExpandingTemplates() {
		$client = new Client('http://test.com', ['test' => 'hi']);
		$ref = new \ReflectionMethod($client, 'expandTemplate');
		$ref->setAccessible(true);
		$uri = $ref->invoke($client, 'http://{test}{?query*}', ['query' => ['han' => 'solo']]);
	...
	public function testCanSetDefaultRequestOptions() {
		$client = new Client();
		$client->getConfig()->set('request.options', [
			'query' => ['test' => '123', 'other' => 'abc'],
			'headers' => ['Foo' => 'Bar', 'Baz' => 'Bam'],
		]);
		$request = $client->createRequest('GET', 'http://www.foo.com?test=hello', ['Foo' => 'Test']);
	...
	public function testCanSetSetOptionsOnRequests() {
		$client = new Client();
		$request = $client->createRequest('GET', 'http://www.foo.com?test=hello', ['Foo' => 'Test'], null, [
			'cookies' => ['michael' => 'test'],
		]);
	...
	public function testClientInjectsConfigsIntoUrls() {
		$client = new Client('http://www.test.com/api/v1', [
			'test' => '123',
		]);
	...
	public function testClientReturnsValidBaseUrls() {
		$client = new Client('http://www.{foo}.{data}/', [
			'data' => '123',
			'foo' => 'bar',
		]);
	...
	public function testClientThrowsExceptionForMultipleRequests() {
		...
		$client->getEventDispatcher()->addSubscriber($mock);
		$client->send([$client->get(), $client->head()]);
	}
	...
	public function testConfigSettingsControlSslConfiguration() {
		...
		$client = new Client('https://www.secure.com/', ['ssl.certificate_authority' => 'system']);
		$this->assertNull($client->getConfig('curl.options'));
		// Can set the cacert value as well
		$client = new Client('https://www.secure.com/', ['ssl.certificate_authority' => false]);
	...
	public function testDescribesEvents() {
		$this->assertEquals(['client.create_request'], Client::getAllEvents());
	}
	...
	public function testExpandsUriTemplatesUsingConfig() {
		$client = new Client('http://www.google.com/');
		$client->setConfig(['api' => 'v1', 'key' => 'value', 'foo' => 'bar']);
		$ref = new \ReflectionMethod($client, 'expandTemplate');
		$ref->setAccessible(true);
	...
	public function testHeadCanUseOptions() {
		$client = new Client();
		$head = $client->head('http://www.foo.com', [], ['query' => ['foo' => 'bar']]);
		$this->assertEquals('bar', $head->getQuery()->get('foo'));
	}

	public function testOverwritesUserAgent() {
		$client = new Client();
		$request = $client->createRequest('GET', 'http://www.foo.com', ['User-agent' => 'foo']);
	...
	public function testUriArrayAllowsCustomTemplateVariables() {
		...
		$this->assertEquals('/hi', (string) $client->createRequest('GET', ['/{var}', $vars])->getUrl());
		$this->assertEquals('/hi', (string) $client->get(['/{var}', $vars])->getUrl());
		$this->assertEquals('/hi', (string) $client->put(['/{var}', $vars])->getUrl());
		$this->assertEquals('/hi', (string) $client->post(['/{var}', $vars])->getUrl());
		$this->assertEquals('/hi', (string) $client->head(['/{var}', $vars])->getUrl());
		$this->assertEquals('/hi', (string) $client->options(['/{var}', $vars])->getUrl());
	}
	...
	public function urlProvider() {
		...
		return [
			[$u, '', $u],
			[$u, 'relative/path/to/resource', $u . 'relative/path/to/resource'],
			[$u, 'relative/path/to/resource?a=b&c=d', $u . 'relative/path/to/resource?a=b&c=d'],
			[$u, '/absolute/path/to/resource', $this->getServer()->getUrl() . 'absolute/path/to/resource'],
			[$u, '/absolute/path/to/resource?a=b&c=d', $this->getServer()->getUrl() . 'absolute/path/to/resource?a=b&c=d'],
			[$u2, '/absolute/path/to/resource?a=b&c=d', $this->getServer()->getUrl() . 'absolute/path/to/resource?a=b&c=d&z=1'],
			[$u2, 'relative/path/to/resource', $this->getServer()->getUrl() . 'base/relative/path/to/resource?z=1'],
			[$u2, 'relative/path/to/resource?another=query', $this->getServer()->getUrl() . 'base/relative/path/to/resource?another=query&z=1'],
		];
	}
