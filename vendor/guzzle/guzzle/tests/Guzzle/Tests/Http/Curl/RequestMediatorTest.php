...
class RequestMediatorTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testEmitsEvents() {
		...
		$request->getEventDispatcher()->addListener('curl.callback.progress', [$this, 'event']);
		$request->getEventDispatcher()->addListener('curl.callback.read', [$this, 'event']);
		$request->getEventDispatcher()->addListener('curl.callback.write', [$this, 'event']);
