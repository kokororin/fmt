...
class HeaderComparison {
	 ...
	 * @return array|bool Returns an array of the differences or FALSE if none
	 */
	public function compareArray(array $expected, $actual, array $ignore = [], array $absent = []) {
		$differences = [];
		...
		foreach ($actual as $key => $value) {
			...
			foreach ((array) $actual[$key] as $v) {
				if ((false === $pos && $v != $expected[$lkey]) || $pos > 0 && substr($v, 0, $pos) != substr($expected[$lkey], 0, $pos)) {
