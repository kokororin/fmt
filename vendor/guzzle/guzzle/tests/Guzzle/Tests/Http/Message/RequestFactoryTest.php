...
class HttpRequestFactoryTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testCanAddBasicAuth() {
		$request = RequestFactory::getInstance()->create('GET', 'http://foo.com', [], null, [
			'auth' => ['michael', 'test'],
		]);
	...
	public function testCanAddCookies() {
		...
		$request = $client->get('/', [], ['cookies' => ['Foo' => 'Bar']]);
	...
	public function testCanAddEvents() {
		$foo = null;
		$client = new Client();
		$client->addSubscriber(new MockPlugin([new Response(200)]));
	...
	public function testCanAddEventsWithPriority() {
		...
		$request = $client->get($this->getServer()->getUrl(), [], [
			'events' => [
				'request.before_send' => [function () use (&$foo) {$foo = true;}, 100],
			],
		]);
		$request->send();
		$this->assertTrue($foo);
	...
	public function testCanChangeSaveToLocation() {
		...
		$client = new Client();
		$request = $client->get('/', [], [
			'plugins' => [new MockPlugin([new Response(200, [], 'testing')])],
			'save_to' => $r,
		]);
		$request->send();
	...
	public function testCanSetVerifyToOn() {
		$client = new Client();
		$request = $client->get('/', [], ['verify' => true]);
	...
	public function testCanSetVerifyToPath() {
		$client = new Client();
		$request = $client->get('/', [], ['verify' => '/foo.pem']);
	...
	public function testClonesRequestsWithMethodWithClient() {
		...
		$client = new Client();
		$request = $client->put('http://www.test.com', ['Content-Length' => 4], 'test');
	...
	public function testClonesRequestsWithMethodWithoutClient() {
		$f = RequestFactory::getInstance();
		$request = $f->create('GET', 'http://www.test.com', ['X-Foo' => 'Bar']);
		$request->getParams()->replace(['test' => '123']);
	...
	public function testCreatesBodyWithoutZeroString() {
		$request = RequestFactory::getInstance()->create('PUT', 'http://test.com', [], '0');
	...
	public function testCreatesNewPostRequestWithFields() {
		...
		$this->assertEquals(['a' => 'b'], $request->getPostFields()->getAll());
		unset($request);

		// Use a collection
		$request = RequestFactory::getInstance()->create('POST', 'http://www.google.com/path?q=1&v=2', null, new Collection([
			'a' => 'b',
		]));
		$this->assertEquals(['a' => 'b'], $request->getPostFields()->getAll());

		// Use a QueryString
		$request = RequestFactory::getInstance()->create('POST', 'http://www.google.com/path?q=1&v=2', null, new QueryString([
			'a' => 'b',
		]));
		$this->assertEquals(['a' => 'b'], $request->getPostFields()->getAll());

		$request = RequestFactory::getInstance()->create('POST', 'http://www.test.com/', null, [
			'a' => 'b',
			'file' => '@' . __FILE__,
		]);
	...
	public function testCreatesProperTransferEncodingRequests() {
		$request = RequestFactory::getInstance()->create('PUT', 'http://www.google.com/', [
	...
	public function testHandlesChunkedTransferEncoding() {
		$request = RequestFactory::getInstance()->create('PUT', 'http://www.foo.com/', [
			'Transfer-Encoding' => 'chunked',
		], 'Test');
		$this->assertFalse($request->hasHeader('Content-Length'));
		$this->assertEquals('chunked', $request->getHeader('Transfer-Encoding'));

		$request = RequestFactory::getInstance()->create('POST', 'http://www.foo.com/', [
			'transfer-encoding' => 'chunked',
		], [
			'foo' => 'bar',
		]);
