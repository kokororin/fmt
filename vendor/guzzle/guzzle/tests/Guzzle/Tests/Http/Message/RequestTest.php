...
class RequestTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testCanSendCustomRequests() {
		$this->getServer()->flush();
		$this->getServer()->enqueue("HTTP/1.1 200 OK\r\nContent-Length: 0\r\n\r\n");
		$request = $this->client->createRequest('PROPFIND', $this->getServer()->getUrl(), [
	...
	public function testConstructorBuildsRequestWithCollectionHeaders() {
		$request = new Request('GET', 'http://www.guzzle-project.com/', new Collection([
			'foo' => 'bar',
		]));
		$this->assertEquals('bar', $request->getHeader('foo'));
	}
	...
	public function testGetResponse() {
		...
		$response = new Response(200, ['Content-Length' => 3], 'abc');
	...
	public function testRequestThrowsExceptionOnBadResponse() {
		try {
			$this->request->setResponse(new Response(404, ['Content-Length' => 3], 'abc'), true);
			$this->request->send();
	...
	public function testRequestsCanBeConvertedToRawMessageStrings() {
		...
		$request = RequestFactory::getInstance()->create('PUT', 'http://www.google.com/path?q=1&v=2', [
			'Authorization' => 'Basic ' . $auth,
		], 'Data');
	...
	public function testUsesCustomResponseBodyWhenItIsCustom() {
		...
		$request->setResponse(new Response(200, [], 'foo'));
