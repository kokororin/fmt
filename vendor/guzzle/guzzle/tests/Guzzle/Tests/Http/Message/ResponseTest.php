...
class ResponseTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function setup() {
		$this->response = new Response(200, new Collection([
			'Accept-Ranges' => 'bytes',
			'Age' => '12',
			'Allow' => 'GET, HEAD',
	...
	public function testDeterminesIfItCanBeCached() {
		...
		$this->assertFalse($this->getResponse(200, [
	...
	public function testDeterminesResponseMaxAge() {
		...
		$this->assertEquals(140, $this->getResponse(200, [
			'Cache-Control' => 's-maxage=140',
		])->getMaxAge());

		// Uses the response's max-age
		$this->assertEquals(120, $this->getResponse(200, [
		...
		$this->assertGreaterThanOrEqual(82400, $this->getResponse(200, [
			'Expires' => gmdate(ClientInterface::HTTP_DATE, strtotime('+1 day')),
		])->getMaxAge());
	...
	public function testGetMessage() {
		$response = new Response(200, new Collection([
			'Content-Length' => 4,
		]), 'body');
	...
	public function testGetSetCookieNormalizesHeaders() {
		...
		$this->assertEquals(
			['UserID=JohnDoe; Max-Age=3600; Version=1', 'foo', 'fubu'],
			$this->response->getHeader('Set-Cookie')->toArray()
		);
	}
	...
	public function testHasTransferInfo() {
		...
		$this->assertEquals([], $this->response->getInfo());
	...
	public function testParsesXmlResponses() {
		$response = new Response(200, [], '<abc><foo>bar</foo></abc>');
