...
class MethodProxyIteratorTest extends \PHPUnit_Framework_TestCase {
	public function testProxiesMagicCallsToInnermostIterator() {
		...
		$proxy->append('a');
		$proxy->append('b');
		$this->assertEquals(['a', 'b'], $i->getArrayCopy());
		$this->assertEquals(['a', 'b'], $proxy->getArrayCopy());
	}

	public function testUsesInnerIterator() {
		$i = new MethodProxyIterator(new ChunkedIterator(new \ArrayIterator([1, 2, 3, 4, 5]), 2));
